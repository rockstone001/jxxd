...
class Location {
    ...
    const SK = '5GQL7GS4Y7qUrYXdpCjzZBdo0d2024nm';
	...
	public function geoII($address)
	{
        $post_string = 'output=json&ak=' . self::AK . '&address=' . $address;
        $data = $this->sock_post(self::URL, $post_string);
        ...
        if (isset($data['status']) && $data['status'] == 0) {
            $rtn = [
                $data['result']['location']['lng'], $data['result']['location']['lat']
            ];
        }
        return $rtn;
	}

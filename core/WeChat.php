...
class WeChat
{
	private $app_id = 'wx385db65142c1d6c2';
	private $app_secret = 'edd506483ba4bd3601d9497a06bb4b97';
	...
	**/
	...
	public function getTicket()
	{
		if (file_exists($this->ticketFile)) {
			...
			} elseif (isset($ticket['expires_in']) && $ticket['expires_in'] > date('U')) {
...
//		https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token=ACCESS_TOKEN&type=jsapi
	...
	private function refreshTicket()
	{
		...
		$loopNum = 0;
		while (isset($ticket['errcode']) && $ticket['errcode'] > 0 && $loopNum < 5) {
	...
	private function refreshAccessToken()
	{
		...
		$loopNum = 0;
		while (isset($accessToken['errcode']) && $accessToken['errcode'] > 0 && $loopNum < 5) {
			sleep(1);
			$accessToken = json_decode($this->getResponse('token', array(
				'grant_type' => 'client_credential', 'appid' => $this->app_id, 'secret' => $this->app_secret
			)), true);
		...
		$accessToken['expires_in'] = date('U') + 7100;
...
//        print_r($tmpArr);
	...
	{
//		$this->getResponse('message/mass/send', )
	}

	public function set_industry($data)
	{
		return $this->getResponse('template/api_set_industry', $this->urlParams, $data);
	}

	public function get_industry()
	{
		return $this->getResponse('template/get_industry', $this->urlParams, true);
	}

	public function get_all_templates()
	{
		return $this->getResponse('template/get_all_private_template', $this->urlParams, true);
	}

	public function template_msg_send($data)
	{
		return $this->getResponse('message/template/send', $this->urlParams, $data);
	}
}

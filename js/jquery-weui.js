...
+ function($) {
  ...
  Datetime.prototype = {
    ...
    getConfig: function() {
      ...
      if(this.params.inputValue && /^\d{4}-\d{1,2}-\d{1,2} \d{1,2}:\d{1,2}$/.test(this.params.inputValue)) {
        var datetimeArray = this.params.inputValue.split(' ');
        var dateArray = datetimeArray[0].split('-');
        var timeArray = datetimeArray[1].split(':');
        today = new Date(dateArray[0], dateArray[1] - 1, dateArray[2], timeArray[0], timeArray[1], 0);
      }

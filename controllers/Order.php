...
class Order extends Base_Controller {
    ...
    public function mine()
    {
        ...
        $user = $this->get_user();
        $this->_success($this->order->get_my_lesson($user['id']));
    }

    public function all($id)
    {
        $this->load->model('Order_model', 'order');
        $this->_success($this->order->get_all_order($id));
    }
...
//            $this->setLayout('blank');
            $this->view('home/order_detail2', $this->order->get_detail($id));
        } else {
...
//        $this->_echo($jsparams);
    ...
    public function cancel_order()
    {
        ...
        if ($order['uid'] != $user['id'] && !$this->is_admin()) {
            $this->_error('400003', '不能取消别人的订单');
        }
        if ($this->order->feidan($post['id'], $order['lesson_id'])) {
            $this->_success();
        } else {
            $this->_error('400003', '取消订单失败');
        }
    }

    public function mylesson($id)
    {
        $this->load->model('Lesson_model', 'lesson');
        $this->load->model('Order_model', 'order');
        $lesson = $this->lesson->get_one([
        ...
        $this->title = '我的报名 : ' . $lesson['title'];

        $this->view('home/order_by_lesson', [
            'orders' => $this->order->get_my_order_by_lesson_id($this->uid, $id)
        ]);
    }
}
